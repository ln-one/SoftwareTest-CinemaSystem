...
本实验以自主开发的电影院售票管理系统为测试对象，采用 JUnit 5 + Mockito 测试框架，对系统后端 Service 层进行了全面的单元测试和安全测试。实验共设计并执行 133 个测试用例，涵盖功能测试、安全测试、边界条件等多种场景。其中安全测试重点验证了认证安全、密码安全、SQL注入防护、XSS防护、授权安全等方面。通过 JaCoCo 工具进行覆盖率统计，Service 层指令覆盖率达到 97%，分支覆盖率达到 93%。在测试过程中发现 2 个代码缺陷和多个潜在安全风险点，并进行了详细的根因分析，提出了相应的修复建议。

**关键词**：安全测试；单元测试；代码覆盖率；SQL注入；认证授权；JaCoCo
...
1. **掌握安全测试的方法** - 学习如何设计和执行安全相关的测试用例
2. 掌握单元测试的设计原则与实现方法
