...
本实验以自主开发的电影院售票管理系统为测试对象，采用 JUnit 5 + Mockito 测试框架，对系统后端 Service 层进行了全面的单元测试。实验共设计并执行 116 个测试用例，涵盖正常流程、异常处理、边界条件等多种场景。通过 JaCoCo 工具进行覆盖率统计，Service 层指令覆盖率达到 97%，分支覆盖率达到 93%。在测试过程中发现 2 个代码缺陷，并进行了详细的根因分析，提出了相应的修复建议。

**关键词**：单元测试；代码覆盖率；JaCoCo；缺陷分析；Spring Boot
...
1. 掌握单元测试的设计原则与实现方法
2. 理解代码覆盖率的概念及度量方式
3. 学习使用 JaCoCo 工具进行覆盖率统计
4. 通过测试用例发现软件缺陷并进行根因分析
5. 培养团队协作完成测试任务的能力
...
本实验选用团队自主开发的**电影院售票管理系统**作为被测对象。该系统是一个完整的电影院在线售票解决方案，采用前后端分离架构，支持用户在线浏览电影、选座购票，以及管理员对影院、电影、场次、订单等信息的管理。
...
|--------|--------|------|------|------|----------|
| SysUserServiceTest | 21 | 21 | 0 | 0 | 0.024s |
| SysBillServiceTest | 15 | 15 | 0 | 0 | 0.015s |
| SysSessionServiceTest | 11 | 11 | 0 | 0 | 0.011s |
| SysMovieServiceTest | 11 | 11 | 0 | 0 | 0.009s |
...
1. **测试用例设计**：完成 116 个单元测试用例的设计与实现，覆盖正常流程、异常处理、边界条件等多种场景

2. **覆盖率达标**：Service 层指令覆盖率达到 97%，分支覆盖率达到 93%，达到了较高的测试充分性

3. **缺陷发现**：通过测试发现 2 个代码缺陷，并进行了详细的根因分析，提出了修复建议

4. **团队协作**：5 名成员分工明确，各自负责不同模块的测试，最终整合形成完整的测试报告

### 8.2 经验总结

1. **Mock 技术的价值**：通过 Mockito 框架隔离外部依赖，使得单元测试快速、稳定、可重复

2. **覆盖率的局限性**：高覆盖率是必要条件但非充分条件，需要结合多种测试设计技术才能发现深层缺陷

3. **边界条件的重要性**：本次发现的 2 个缺陷都是通过边界条件测试发现的，说明边界值分析的重要性

4. **测试即文档**：良好的测试用例命名和注释，可以作为代码行为的文档

### 8.3 改进建议

1. **扩展测试范围**：后续可增加 Controller 层的集成测试和 Mapper 层的数据库测试

2. **引入静态分析**：使用 SonarQube 等工具进行静态代码分析，提前发现潜在问题

3. **持续集成**：将测试集成到 CI/CD 流程中，每次提交自动运行测试

4. **性能测试**：针对高并发场景（如抢票）进行性能测试
